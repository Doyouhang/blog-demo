...
export const TYPES = {
  ...
  items: {
    label: '收藏',
    hint: '听过的歌、读过的书。想细说的另写长文，在这里关联过去。',
    ...
    fields: [
      { key: 'kind', label: '类型', type: 'select', required: true,
        options: [{ v: 'book', t: '书' }, { v: 'song', t: '歌' }] },
      { key: 'title', label: '标题', type: 'text', required: true },
      { key: 'creator', label: '作者 / 歌手', type: 'text', required: true },
      { key: 'date', label: '听到 / 读完的时间', type: 'date', required: true },
      { key: 'status', label: '状态', type: 'select', onlyWhen: { kind: 'book' },
        options: [{ v: '', t: '（不填，按读完算）' }, { v: 'doing', t: '在读' },
                  { v: 'done', t: '读完' }, { v: 'want', t: '想读' }] },
